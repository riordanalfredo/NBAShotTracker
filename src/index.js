...
import Pie from "./pie";
import classRoomSvg from "../classrooms/classroom.svg";
...
function randomButton() {
  ...
  d3.selectAll(".lucky")
    ...
    .on("click", function (d, i) {
      d3.csv(`./dataset/${season.toString()}.csv`).then(function (data) {
...
const debounceSearch = debounce(
  (event) => playerMenu(event, CONSTANTS.defaultSeason),
  300
);
...
export function drawChart(playerName, season, date, period) {
  ...
  // .append("svg")
  // .attr("width", CONSTANTS.courtWidth)
  // .attr("height", CONSTANTS.courtHeight);

  d3.xml(classRoomSvg).then((data) => {
    svgContainer.node().append(data.documentElement);
    // let svg = svgContainer.append("svg");
    // let court = new Court(svg);
    // court.render();

    new Shots(svgContainer.select("svg"), playerName, season, date, period);
  });
...
function playerMenu(searchText, season) {
  ...
  d3.csv(`./dataset/${season}.csv`).then(function (data) {
    ...
    data.forEach((player) => {
      ...
      if (
        (player.name.slice(0, searchLength).toLowerCase() ===
          searchText.toLowerCase() ||
          lastName.slice(0, searchLength).toLowerCase() ===
            searchText.toLowerCase()) &&
        Object.keys(players).length <= 6 &&
        !Object.keys(players).includes(player.name)
      ) {
...
function loadPlayerGames(player, season) {
  d3.csv(`./dataset/${season}.csv`).then(function (data) {
    ...
    data.forEach((shot) => {
      if (
        shot.name.toLowerCase() === player.toLowerCase() &&
        games[shot.game_date] === undefined
      ) {
...
function displayPlayerTeam(team) {
  ...
  d3.select(".teamseason")
    .append("h3")
    .text(`Search by Season`)
    .classed("seasonlabel");
}
...
function displaySeasonSelector(selectedOption = 2017) {
  ...
  d3.select(".seasonselect-div select").on("change", function (d, i) {
    const playerName = d3.select(".searchfield")._groups[0][0].placeholder;
...
function getTeamname() {
  const teamName = d3
    .select(".teamseason h3")
    ._groups[0][0].textContent.split(" ");
...
function displayGameBreakdownButton(playerName, season) {
  ...
  d3.select("#svgcontainer").append("div").attr("class", "breakdownloading");
